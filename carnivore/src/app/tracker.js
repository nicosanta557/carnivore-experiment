...
const ENTRIES = [
  {
    ...
    carbs: 0,
    calories: 1950,
    note: 'Day 1 in the books. Kept it simple — steak and eggs. Energy felt surprisingly steady through the afternoon. No sugar crashes. A little foggy by 7pm but pushed through. Water intake was solid.',
  },
  // Add more entries below as days go on:
  // {
  //   date: '2026-04-03',
  //   day: 2,
  //   weight: 248.5,
...
function DayCard({ entry }) {
  ...
  const delta = entry.weight != null ? (entry.weight - START_WEIGHT).toFixed(1) : null
  return (
...
export default function CarnivoreTracker() {
  ...
  const lostLbs = currentWeight != null ? (START_WEIGHT - currentWeight).toFixed(1) : null
  const today = Math.floor((new Date() - START_DATE) / 86400000) + 1
    ...
    <div style={{ minHeight: '100vh', background: '#faf9f7', fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif" }}>
      ...
      <div style={{ maxWidth: '700px', margin: '0 auto', padding: '36px 18px 80px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px', marginBottom: '36px' }}>
          <Stat label="Starting Weight" value={`${START_WEIGHT} lbs`} />
